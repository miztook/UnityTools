...
local CSummonPagePetEgg = Lplus.Extend(CSummonPageBase, "CSummonPagePetEgg")
local def = CSummonPagePetEgg.define
...
def.static("=>", CSummonPagePetEgg).new = function()
	local PageEggShop = CSummonPagePetEgg()
...
def.override("dynamic").OnData = function(self, data)
    ...
    game._CGuideMan:AnimationEndCallBack(self._PanelSummon)
end
...
def.override().PlayVideoBG = function (self)
    GameUtil.ActivateVideoUnit(self._PanelSummon._VideoPlayer_Pet, self._PanelSummon._Img_Screen_Video)
end

def.override("=>", "string").GetSummonPageTemplateName = function(self)
    return "Page_MallPetEgg"
end
...
def.method().ShowExchangePanel = function(self)
    ...
    for i,v in ipairs(self._ExchangePetItemsData) do
        local itemTemp = CElementData.GetItemTemplate(v.ItemTid)
        if itemTemp == nil then  warn("item id ".. v.ItemTid.." is nil") return end
        GUITools.SetItemIcon(self._PanelObjects.ImgExchangePetIcon[i],itemTemp.IconAtlasPath)
        GUI.SetText(self._PanelObjects.LabNameExchange[i], RichTextTools.GetItemNameRichText(v.ItemTid, 1, false))
        ...
        local dropRuleId = v.DropRuleTid
        local dropRuleTemplate = CElementData.GetTemplate("DropRule",dropRuleId)
        if dropRuleTemplate == nil then warn(" droupRule id".. dropRuleId .." is nil") return end
...
def.override().OnHide = function(self)
    GameUtil.DeactivateVideoUnit(self._PanelSummon._VideoPlayer_Pet)
end
...
CSummonPagePetEgg.Commit()
return CSummonPagePetEgg