...
local CPanelMainTipsLow = Lplus.Extend(CPanelBase, 'CPanelMainTipsLow')
local def = CPanelMainTipsLow.define
...
def.field("string")._DoTweenID_BattleStg1 = "1"
def.field("string")._DoTweenID_BattleStg2 = "2"
...
local instance = nil
def.static('=>', CPanelMainTipsLow).Instance = function ()
	if not instance then
		instance = CPanelMainTipsLow()
		instance._PrefabPath = PATH.Panel_MainTips_Low
...
--		self._DTP_FSDetail:Stop(FS_DETAIL_DT_GRP)
--		self._DTP_FSDetail:GoToEndPos(FS_DETAIL_DT_GRP)
...
local function CloseBattleStgTip()
	if not IsNil(CPanelMainTipsLow.Instance()._BattleStgTip) then
		--warn("CloseBattleStgTip")
		local Img_Bg = CPanelMainTipsLow.Instance()._BattleStgTip:FindChild('Img_Bg')
		GameUtil.StopUISfx(PATH.UIFX_BOSS_Warning, Img_Bg)
		CPanelMainTipsLow.Instance()._BattleStgTip:SetActive(false)
	end
end
...
def.method("string","number").PopSkillTip = function(self, content, dur)
	...
	if not IsNil(self._BattleStgTip) then
		...
		self._BattleStg_TweenPlayer:Restart(self._DoTweenID_BattleStg1)
		-- self:AddEvt_PlayDotween(EvtGroupName_SkillTip, 0, self._BattleStg_TweenPlayer, self._DoTweenID_BattleStg1)
		local Img_Bg = self._BattleStgTip:FindChild('Img_Bg')
		-- GameUtil.PlayUISfx(PATH.UIFX_BOSS_Warning, Img_Bg, Img_Bg, -1)
		self:AddEvt_PlayFx(EvtGroupName_SkillTip, 1, PATH.UIFX_BOSS_Warning, Img_Bg, Img_Bg, -1, 1)
		self:AddEvt_PlayDotween(EvtGroupName_SkillTip, dur + 1, self._BattleStg_TweenPlayer, self._DoTweenID_BattleStg2)
		self:AddEvt_LuaCB(EvtGroupName_SkillTip, dur + 2, CloseBattleStgTip)
	end
end 
...
--	self._ManualTipsLab = nil
	...
	self._BattleStgTip = nil
	self._BattleStgTxt = nil
	self._BattleStg_TweenPlayer = nil
...
CPanelMainTipsLow.Commit()
return CPanelMainTipsLow