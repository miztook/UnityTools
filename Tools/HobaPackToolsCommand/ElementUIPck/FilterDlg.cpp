...
CPathTreeNode* CPathTree::InsertItem(CPathTreeNode *pParent, CString txt, BOOL bCheck)
{
	if(pParent==0)
	{
		if(mRoot==0)
...
void CFilterDlg::ReadPathTree(CString path, HTREEITEM hParent, int iCount, bool bInit /*= false*/, bool bOnlyFirst /*= false*/)
{
	if(iCount <= 0) 
		return;
...
HTREEITEM CFilterDlg::GetInsertPos(HTREEITEM hParent,CString name, bool bFolder /*= true*/)
{
