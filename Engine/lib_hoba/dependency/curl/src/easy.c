...
CURLcode curl_easy_setopt_vargs(CURL *curl, CURLoption option, va_list arg)
{
	struct SessionHandle *data = curl;
	...
	return Curl_setopt(data, option, arg);
}
...
CURLcode curl_easy_getinfo_vargs(CURL *curl, CURLINFO info, va_list arg)
{
	void *paramp;
	struct SessionHandle *data = (struct SessionHandle *)curl;
	paramp = va_arg(arg, void *);
