...
static bool _MovingAABBToBrush(AABBTRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane);
static bool _StaticAABBToBrush(AABBTRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane);
static bool _MovingSphereToBrush(SPHERETRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane);
static bool _StaticSphereToBrush(SPHERETRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane);
					   ...
					   const A3DVECTOR3& vVelocity, float fTime, float fEpsilon/* 0.03125f */)
{
	...
	pInfo->BoundAABB.Mins = a3d_VecMin(vDest - vExts, vStart - vExts) - A3DVECTOR3(0.5f);
	pInfo->BoundAABB.Maxs = a3d_VecMax(vDest + vExts, vStart + vExts) + A3DVECTOR3(0.5f);
...
*/
bool TRA_AABBMoveToBrush(AABBTRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	if (!iNumPlane)
		return false;
...
bool TRA_AABBMoveToOBB(AABBTRACEINFO* pInfo, const A3DOBB& obb)
{
	...
	return TRA_AABBMoveToBrush(pInfo, (A3DPLANE**)Bevels.aPlaneAddrs, Bevels.iNumPlane);
}
...
*/
bool _MovingAABBToBrush(AABBTRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	int iClipPlane = -1;
	...
	{
		const A3DPLANE* pPlane = aPlanes[i];
...
/*	Collision routine of static AABB and brush (convex polyhedron)
	...
	iNumPlane: number of plane pointer in aPlanes
*/
bool _StaticAABBToBrush(AABBTRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	pInfo->fFraction	= 1.0f;
	pInfo->bStartSolid	= false;
	pInfo->bAllSolid	= false;
...
bool TRA_SphereMoveToBrush(SPHERETRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	if (!iNumPlane)
		return false;
...
bool _MovingSphereToBrush(SPHERETRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	int iClipPlane = -1;
	float fEnterFrac = -1e7f, fLeaveFrac = 1e7f, fFrac;
	bool bGetOut = false, bStartOut = false;
...
bool _StaticSphereToBrush(SPHERETRACEINFO* pInfo, A3DPLANE** aPlanes, int iNumPlane)
{
	pInfo->fFraction	= 1.0f;
	pInfo->bStartSolid	= false;
	pInfo->bAllSolid	= false;
