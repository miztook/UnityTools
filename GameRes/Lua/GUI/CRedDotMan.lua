...
local function _UpdateModuleRedDotShow(type,state) 
    ...
    if RedDotSystemTabel[type] then
	    ...
	    if type < 5 or type > 18 then return end
	    _UpdateSystemMenuButtonShow()
    end

    -- 发送主界面红点更新的事件
    local MainPanelRedPointStateChangeEvent = require "Events.MainPanelRedPointStateChangeEvent"
    local event = MainPanelRedPointStateChangeEvent()
    event._RedDotType = type
    event._State = state
