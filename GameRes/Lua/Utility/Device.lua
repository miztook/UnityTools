...
local android_native_lib_name = "相应的类库"
--[[--
...
function device.ShowActivityIndicator()
    ...
    if device.platform == "android" then
        luaj.callStaticMethod(android_native_lib_name, "showActivityIndicator", {}, "()V"); 
...
function device.HideActivityIndicator()
    ...
    if device.platform == "android" then
        luaj.callStaticMethod(android_native_lib_name, "hideActivityIndicator", {}, "()V"); 
    elseif device.platform == "ios" then
...
function device.ShowAlert(title, message, buttonLabels, listener)
	...
	if device.platform == "android" then
		...
		luaj.callStaticMethod(android_native_lib_name, "createAlert", {title, message, buttonLabels, tempListner}, "(Ljava/lang/String;Ljava/lang/String;Ljava/util/Vector;I)V");
	else
	    local defaultLabel = ""
...
function device.vibrate(millisecond)
    ...
    if device.platform == "android" then
        if millisecond then
            luaj.callStaticMethod("android_native_lib_name", "vibrate", {millisecond}, "(I)V");
        else      
            CCNative:vibrate()
        end     
    elseif device.platform == "ios" then
