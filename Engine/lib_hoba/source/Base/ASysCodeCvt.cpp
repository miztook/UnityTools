...
aint32 ASysCodeCvt::ParseUnicodeFromUTF8Str(const char* szUTF8, aint32* pnAdvancedInUtf8Str, auint32 nUtf8StrMaxLen)
{
	auint32 nAdv = 0;
	aint32 nResult = 0;
...
aint32 ASysCodeCvt::ParseUnicodeFromUTF8StrEx(const char* szUTF8, aint32 iParsePos, aint32* piParsedHeadPos, aint32* pnUtf8ByteCounts, auint32 nUtf8StrMaxLen)
{
	aint32* pnAdvancedInUtf8Str = pnUtf8ByteCounts;
	auint32 nAdv = 0;
	aint32 nResult = 0;
	if (pnAdvancedInUtf8Str != 0)
		*pnAdvancedInUtf8Str = 0;
	...
	if (iParsePos < 0)
		return 0;
	if (nUtf8StrMaxLen <= (auint32)iParsePos)
		return 0;

	aint32 nLen = (aint32)strnlen(szUTF8, nUtf8StrMaxLen);
	if (nLen <= iParsePos)
		return 0;
...
ASysCodeCvt::UTF8_EncodedChar ASysCodeCvt::EncodeUTF8(aint32 ch)
{
...
aint32 ASysCodeCvt::UTF8ToUTF16LE(auchar* sz16, const char* sz8)
{
	if (sz8 == 0)
		return 0;
	int count = 0;
	...
	{
		aint32 nAdv;
		aint32 value = ParseUnicodeFromUTF8Str(sz8, &nAdv);
		if (nAdv == 0)
			break;
		sz8 += nAdv;
		if (value < 0) continue;
...
aint32 ASysCodeCvt::UTF16LEToUTF8(char* sz8, const auchar* sz16)
{
	if (sz16 == 0)
		return 0;
	int count = 0;
...
aint32 ASysCodeCvt::UTF16Len(const auchar* sz16)
{
	const auchar* p = sz16;
	if (p == 0) return 0;
	while (*p != 0)
		++p;
	return (aint32)(p - sz16);
}