...
#ifndef HDETOURCROWD_H
#define HDETOURCROWD_H

#include "HDetourNavMeshQuery.h"
#include "HDetourObstacleAvoidance.h"
#include "HDetourLocalBoundary.h"
#include "HDetourPathCorridor.h"
#include "HDetourProximityGrid.h"
#include "HDetourPathQueue.h"
...
namespace HOBA
{
	...
	/// @ingroup crowd
	static const int DT_CROWDAGENT_MAX_NEIGHBOURS = 49;
	...
	class dtCrowd
	{
		...
		void updateTopologyOptimization(const float dt, dtCrowdAgent* thisAgent) const;
		void updateMoveRequest(const float dt, dtCrowdAgent* thisAgent);
		void checkPathValidity(const float dt, dtCrowdAgent* thisAgent) const;
		bool requestMoveTargetReplan(dtCrowdAgent* thisAgent, dtPolyRef ref, const float* pos) const;
	...
	public:
		...
		bool init(const int maxAgents, const float maxAgentRadius, float fHeight, dtNavMesh* nav);
		...
		///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
		///  @param[in]		pos		The requested position of the agent. [(x, y, z)]
		void updateAgentPos(const int idx, const float* pos);
		...
		///
		void updateAgentMaxSpeed(const int idx, float maxspeed);

		///
		bool updateOneAgent(const int idx, const float dt, bool bIgnoreCollision, bool bKeepSpeed);
