...
static int varint_encoder(lua_State *L)
{
	...
	if (is64BitsStr)
	{
		value = check64BitStringValue(L, index + 1);
	}
	else
	{
		lua_Number l_value = luaL_checknumber(L, index + 1);
		svalue = (int64_t)l_value;
		if (svalue < 0)
			bSigned = true;
		else
...
static int64_t getSignedNumberValue(lua_State *L, int index)
{
	if (lua_type(L, index) == LUA_TSTRING)
	{
		return check64BitStringValue(L, index);
...
{
	int64_t value = getSignedNumberValue(L, 1);
