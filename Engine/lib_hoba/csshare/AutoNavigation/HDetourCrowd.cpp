...
#include "HDetourObstacleAvoidance.h"
#include "HDetourCommon.h"
#include "HDetourMath.h"
#include "HDetourAssert.h"
#include "HDetourAlloc.h"
...
namespace HOBA
{
		...
		dtCrowdAgent** agents, const int /*nagents*/, dtProximityGrid* grid)
	{
		...
		static const int MAX_NEIS = DT_CROWDAGENT_MAX_NEIGHBOURS;
	...
	*/
	...
	bool dtCrowd::init(const int maxAgents, const float maxAgentRadius, float fHeight, dtNavMesh* nav)
	{
		purge();
		...
		//dtVset(m_ext, m_maxAgentRadius*2.0f, m_maxAgentRadius*30.0f, m_maxAgentRadius*2.0f);
		dtVset(m_ext, m_maxAgentRadius*2.0f, fHeight, m_maxAgentRadius*2.0f);
		...
		if (!m_obstacleQuery->init(DT_CROWDAGENT_MAX_NEIGHBOURS + 0, 8))
			return false;
	...
	void dtCrowd::updateAgentPos(const int idx, const float* pos)
	{
		if (idx < 0 || idx >= m_maxAgents)
			return;
	...
	bool dtCrowd::requestMoveTargetReplan(dtCrowdAgent* thisAgent, dtPolyRef ref, const float* pos) const
	{
		if (!ref)
			return false;

		dtCrowdAgent* ag = thisAgent;
	...
	void dtCrowd::updateMoveRequest(const float dt, dtCrowdAgent* thisAgent)
	{
		const int PATH_MAX_AGENTS = 8;
		dtCrowdAgent* queue[PATH_MAX_AGENTS];
		int nqueue = 0;
	...
	void dtCrowd::updateTopologyOptimization(const float dt, dtCrowdAgent* thisAgent) const
	{
		const float OPT_TIME_THR = 0.5f; // seconds
		const int OPT_MAX_AGENTS = 1;
		dtCrowdAgent* queue[OPT_MAX_AGENTS];
	...
	void dtCrowd::checkPathValidity(const float dt, dtCrowdAgent* thisAgent) const
	{
		static const int CHECK_LOOKAHEAD = 10;
		static const float TARGET_REPLAN_DELAY = 1.0; // seconds
	...
	void dtCrowd::updateAgentMaxSpeed(const int idx, float maxspeed)
	{
		if (idx < 0 || idx >= m_maxAgents)
			return;
		m_agents[idx].params.maxSpeed = maxspeed;
	}

	bool dtCrowd::updateOneAgent(const int idx, const float dt, bool bIgnoreCollision, bool bKeepSpeed)
	{
		if (idx < 0 || idx >= m_maxAgents)
			return false;

		dtCrowdAgent* thisAgent = &m_agents[idx];
		...
		//back all active agents
		// 	for (int i = 0; i < nagents; ++i)
		// 	{
		// 		m_tmpAgents[i] = *agents[i];
		// 	}

		// Check that all agents still have valid paths.
		checkPathValidity(dt, thisAgent);

		// Update async move request and path finder.
		updateMoveRequest(dt, thisAgent);

		// Optimize path topology.
		updateTopologyOptimization(dt, thisAgent);

		// Register agents to proximity grid.
		float thisminx = thisAgent->npos[0] - thisAgent->params.collisionQueryRange;
		float thisminz = thisAgent->npos[2] - thisAgent->params.collisionQueryRange;
		float thismaxx = thisAgent->npos[0] + thisAgent->params.collisionQueryRange;
		float thismaxz = thisAgent->npos[2] + thisAgent->params.collisionQueryRange;
		...
		for (int i = 0; i < nagents; ++i)
		{
			...
			if (p[0] - r >= thismaxx || p[2] - r >= thismaxz || p[0] + r <= thisminx || p[2] + r <= thisminz)
				continue;
		...
		do
		{
			...
			// 			if (ag != thisAgent)				//ONLY THIS AGENT!
			// 				continue;
		...
		*/
		...
		//restore all active agents, except thisAgent
		// 	for (int i = 0; i < nagents; ++i)
		// 	{
		// 		if (thisAgent != agents[i])
		// 			*agents[i] = m_tmpAgents[i];
		// 	}
