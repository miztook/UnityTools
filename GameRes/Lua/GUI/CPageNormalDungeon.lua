...
local CPageNormalDungeon = Lplus.Class("CPageNormalDungeon")
local def = CPageNormalDungeon.define
...
def.field('table')._N_DungeonTemplates = BlankTable -- 所有普通遗迹的模版
def.field("table")._SfxTimerIdMap = BlankTable
-- 通用
def.field("table")._RewardsData = BlankTable -- 奖励数据

def.static("table", "=>", CPageNormalDungeon).new = function(panel_script)
	local obj = CPageNormalDungeon()
...
def.method().Init = function (self)
    ...
    for _, v in ipairs(allDungeonTid) do
        ...
        if template ~= nil then
            if template.InstanceType == EInstanceType.INSTANCE_RUINS then
                -- 普通遗迹类型
                self._N_DungeonTemplates[#self._N_DungeonTemplates+1] = template
            end
        end
    end
    table.sort(self._N_DungeonTemplates, function (a, b)
        -- 按Id从小到大
        return a.Id < b.Id
    end)
end
...
def.method("dynamic").Show = function(self, data)
    ...
    if next(self._N_DungeonTemplates) == nil then
        warn("普通遗迹的模版数据为空")
        return
    end
    ...
    if type(data) == "number" then
        -- 指定副本，若不属于遗迹普通，打开默认
        for i, v in ipairs(self._N_DungeonTemplates) do
            if v.Id == data then
    ...
    if self._SelectedInstance == -1 then
        selectedIndex = 1
        -- 默认打开已解锁中Id最大的
        for i = #self._N_DungeonTemplates, 1, -1 do
            local id = self._N_DungeonTemplates[i].Id
        ...
        self._SelectedInstance = self._N_DungeonTemplates[selectedIndex].Id
    end

    self._Panel:SetRuinList(#self._N_DungeonTemplates, selectedIndex - 1)
...
def.method("userdata", "string", "number").OnPanelInitItem = function(self, item, id, index)
    if string.find(id, "List_Dungeon") then
        -- 左列表
        local template = self._N_DungeonTemplates[index+1]
        if template ~= nil then
...
def.method("number").UpdateDungeonInfo = function (self, id)
	...
	if instanceTemp == nil then
		warn("Instance template get nil on page NormalDungeon, wrong tid:" .. id)   
		return
	end
    self._Panel:ShowDungeonInfo(id)
    if instanceTemp.RewardId <= 0 then
        warn("Reward template get nil on page NormalDungeon, wrong tid:" .. instanceTemp.RewardId)
    else
...
CPageNormalDungeon.Commit()
return CPageNormalDungeon