...
function crypto.encryptAES256(plaintext, key)
    ...
    key = tostring(key)
    return Crypto:encryptAES256(plaintext, string.len(plaintext), key, string.len(key))
end
...
function crypto.decryptAES256(ciphertext, key)
    ...
    key = tostring(key)
    return Crypto:decryptAES256(ciphertext, string.len(ciphertext), key, string.len(key))
end
...
function crypto.encryptXXTEA(plaintext, key)
    plaintext = tostring(plaintext)
    key = tostring(key)
    return Crypto:encryptXXTEA(plaintext, string.len(plaintext), key, string.len(key))
end
...
function crypto.decryptXXTEA(ciphertext, key)
    ciphertext = tostring(ciphertext)
    key = tostring(key)
    return Crypto:decryptXXTEA(ciphertext, string.len(ciphertext), key, string.len(key))
end
...
function crypto.encodeBase64(plaintext)
    plaintext = tostring(plaintext)
    return Crypto:encodeBase64(plaintext, string.len(plaintext))
end
...
function crypto.md5(input, isRawOutput)
    ...
    return Crypto:MD5(input, isRawOutput)
end
