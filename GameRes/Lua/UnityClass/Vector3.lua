...
function Vector3:Get()
	if self == nil then
		warn("Vector3 self is nil", debug.traceback())
		return 0, 0, 0
	end
...
function Vector3.Distance_XYZ(vaX, vaY, vaZ, vbX, vbY, vbZ)
	return sqrt((vaX - vbX) * (vaX - vbX) + (vaY - vbY) * (vaY - vbY) + (vaZ - vbZ) * (vaZ - vbZ))
end

function Vector3.SqrDistance(va, vb)
	return (va.x - vb.x) * (va.x - vb.x) + (va.y - vb.y) * (va.y - vb.y) + (va.z - vb.z) * (va.z - vb.z)
end

function Vector3.SqrDistance_XYZ(vaX, vaY, vaZ, vbX, vbY, vbZ)
	return (vaX - vbX) * (vaX - vbX) + (vaY - vbY) * (vaY - vbY) + (vaZ - vbZ) * (vaZ - vbZ)
end

function Vector3.DistanceH(va, vb)
	return sqrt((va.x - vb.x) * (va.x - vb.x) + (va.z - vb.z) * (va.z - vb.z))
end

function Vector3.DistanceH_XZ(x1, z1, x2, z2)
	return sqrt((x1 - x2) * (x1 - x2) + (z1 - z2) * (z1 - z2))
end

function Vector3.SqrDistanceH(va, vb)
	return (va.x - vb.x) * (va.x - vb.x) + (va.z - vb.z) * (va.z - vb.z)
end

function Vector3.SqrDistanceH_XZ(x1, z1, x2, z2)
