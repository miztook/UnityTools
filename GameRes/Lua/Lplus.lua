...
--统计
local startTime = 0
local typeCreationTable = nil

_G.StartRecordCreation = function ()
	typeCreationTable = {}
	startTime = os.time()
end

_G.StopRecordCreation = function (filename)
	if typeCreationTable == nil then return end

	local sorted = { }
    for k, v in pairs(typeCreationTable) do
    ...
    file:write("type, creation_count\n")
    ...
    typeCreationTable = nil
end

local RecordCreation = function (theType)
	if typeCreationTable == nil then return end
	
	local item = typeCreationTable[theType]
    if not item then
        typeCreationTable[theType] = { theType, 1 }
    else
        item[2] = item[2] + 1
    end
end
...
local function createForwardDeclareType (typeName, errorLevel)
	...
	local typeString = (typeName or "anonymous type") .. "(forward declare)"
	local typeMeta =
	{
...
local function createType (baseTypeTable, typeNameOrForwardDeclare, isInterface)
	...
	do
		...
		local typeString = (typeName or "anonymous type") .. "(" .. tostring(theType) .. ")"
	...
	local function commit ()
		...
		if feature_accessing_checking then
			...
			function typeMeta.__call (_, ...)
				...
				--[[
				--检查Lplus对象的分配
				local typeString = tostring(theType)
				warn("typeMeta.__call:", typeString)
				if string.find(typeString, "UIShortCutEvent") then
				...
				RecordCreation(tostring(theType))
...
do
	...
	def.method("=>", "string").getName = function (self)
		return getTypeTableMetaNoCheck(self.m_typeTable).typeName
	end
