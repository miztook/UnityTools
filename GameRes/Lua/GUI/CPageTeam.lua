...
def.method().UpdateAllMemberDeadState = function(self)
	local memberList = self._TeamMan:GetMemberListExceptHost()
...
def.method("table").UpdateDeadState = function(self, data)
	...
	local info = CTeamMan.Instance():GetMemberInfoById(data.roleId)
    ...
    local member = self._TeamMan:GetMemberInfoById(data.roleId)
	...
	if data.DeadState then
		...
		GameUtil.SetImageColor(imgHead, color)
		...
		if bIsInVision then
            ...
            GameUtil.SetTextColor(lab_level:GetComponent(ClassType.Text), text_color)
            GameUtil.SetTextColor(lab_name:GetComponent(ClassType.Text), text_color)
            GameUtil.SetImageColor(imgHeadBack, text_color)
            GameUtil.SetImageColor(imgHead, text_color)
        end
...
def.method("userdata", "table").SetItemInfo = function(self, item, memberInfo)
	...
	local lab_level = item:FindChild("Img_LevelBroad/Lab_Level")
	local str = string.format(StringTable.Get(10641), memberInfo._Lv)
	GUI.SetText(lab_level, str)
...
--def.method().CancleMatch = function(self)
--    self._TeamMan:C2STeamCancelMatchReq()
--    if self._MatchTimer ~= 0 then
--        _G.RemoveGlobalTimer(self._MatchTimer)
--        self._MatchTimer = 0
--    end
...
--    if CPVPAutoMatch.Instance():IsMatching(EnumDef.AutoMatchType.SearchTeam) then
--        -- self:UpdateTargetTab()
--    else
--	    self:ResetTeamMemberList()
--	    self:ResetItemList()
--	    self:UpdateItemList()
--	    self:UpdateAllMemberDeadState()
--    end
    self:ResetTeamMemberList()
	self:ResetItemList()
	self:UpdateItemList()
...
def.method().ResetTeamMemberList = function(self)
	...
	self._TeamMemberList = self._TeamMan:GetMemberListExceptHost()
end
